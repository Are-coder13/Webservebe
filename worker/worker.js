...
function reviewSystem() {
  return [
    ...
    '7. MOTION QUALITY & LIVENESS — CRITICAL: is the 3D scene alive AT REST (before any scroll), or frozen until you scroll? The hero must animate continuously every frame independent of scroll — idle rotation of the particle universe/strand/centrepiece + a breathing bob (RECIPE F2), and the hero product silhouette (the tooth/gem/loaf, RECIPE G) must visibly rotate/shimmer/float at all times (RECIPE G2). If the scene\'s ONLY motion is the scroll timeline or mouse parallax (dead when still), ADD a THREE.Clock-driven idle animation so it is never static. Also: scrubbed camera timeline with eased follow + mouse parallax; chapter content reveals on scroll; transitions 150-300ms; nothing janky.',
